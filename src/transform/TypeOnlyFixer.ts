...
type ImportDeclarationWithClause = ts.ImportDeclaration &
  Required<Pick<ts.ImportDeclaration, 'importClause'>>
type ExportDeclarationWithClause = ts.ExportDeclaration &
  Required<Pick<ts.ExportDeclaration, 'exportClause'>>
...
export class TypeOnlyFixer {
  ...
  private fixTypeOnlyImport(node: ImportDeclarationWithClause) {
    ...
    if (namedBindings && ts.isNamedImports(namedBindings)) {
      ...
      if (valueNames.length) {
        valueImports.push(
          `import { ${valueNames.join(', ')} } from ${specifier};`,
        )
      }
    }
  ...
  private fixTypeOnlyExport(node: ExportDeclarationWithClause) {
    ...
    if (ts.isNamedExports(node.exportClause)) {
      ...
      if (typeNames.length) {
        typeExports.push(
          `export type { ${typeNames.join(', ')} }${specifier ? ` from ${specifier}` : ''};`,
        )
      }
      if (valueNames.length) {
        valueExports.push(
          `export { ${valueNames.join(', ')} }${specifier ? ` from ${specifier}` : ''};`,
        )
      }
    }
