...
function preProcessNamespaceBody(
  ...
  if (ts.isModuleDeclaration(body)) {
    if (
      body.body &&
      (ts.isModuleBlock(body.body) || ts.isModuleDeclaration(body.body))
    ) {
  ...
  for (const stmt of body.statements) {
    ...
    if (
      ...
      stmt.body &&
      (ts.isModuleBlock(stmt.body) || ts.isModuleDeclaration(stmt.body))
    ) {
...
export function preProcess({
  ...
  for (const node of sourceFile.statements) {
    ...
    if (ts.isImportDeclaration(node)) {
      ...
      if (ts.isModuleDeclaration(node)) {
        if (
          node.body &&
          (ts.isModuleBlock(node.body) || ts.isModuleDeclaration(node.body))
        ) {
  ...
  const eofTrivia = ts.getLeadingCommentRanges(
    fullText,
