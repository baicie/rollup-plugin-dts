...
export const transform = (enableSourcemap: boolean) => {
  ...
  return {
    ...
    transform(code, fileName, inputMapTextOrOptions?: string | {}) {
      ...
      const moduleId = Array.from(moduleIds).find(
        id => trimExtension(id) === name,
      )
      ...
      const inputMapText =
        typeof inputMapTextOrOptions === 'string'
          ? inputMapTextOrOptions
          : undefined
      ...
      const map = preprocessed.code.generateMap({
        hires: true,
        source: fileName,
      })
      ...
      return {
        ...
        map: map as unknown as SourceMap,
      }
    },
    ...
    renderChunk(inputCode, chunk, options, meta) {
      ...
      for (const fileName of Object.keys(chunk.modules)) {
        ...
        for (const ref of allFileReferences.get(
          ...
          if (ref.startsWith('.')) {
            ...
            const absolutePathToOriginal = path.join(
            ...
            let targetRelPath = path
              .relative(chunkFolder, absolutePathToOriginal)
              .split('\\')
              .join('/')
            if (targetRelPath[0] !== '.') {
      ...
      const moduleDeclarationFixer = new ModuleDeclarationFixer(
        chunk,
        'magicCode' in typesFixed && typesFixed.magicCode
          ? typesFixed.magicCode
    ...
    async generateBundle(options, bundle) {
        ...
        if (inputMap && inputMap.sources) {
          ...
          inputSourcemaps.set(fileName, {
            ...
            sources: inputMap.sources.map(source => {
              ...
              if (sourceRootIsUrl) {
                const base = sourceRoot.endsWith('/')
                  ? sourceRoot
                  : sourceRoot + '/'
      ...
      const outputDir =
        options.dir ||
        (options.file ? path.dirname(options.file) : process.cwd())
      ...
      for (const chunk of Object.values(bundle)) {
        ...
        for (const source of chunk.map.sources) {
          ...
          if (
            !inputMap &&
            /\.[cm]?[tj]sx?$/.test(absoluteSource) &&
            !absoluteSource.endsWith('.d.ts')
          ) {
        ...
        if (sourcesToRemap.size === 0) {
          ...
          if (chunk.map.sources.length === 0 && chunk.facadeModuleId) {
            ...
            if (inputMap && inputMap.sources.length > 0) {
              const newSources = inputMap.sources.map(
        ...
        const isSingleSource =
          chunk.map.sources.length === 1 && sourcesToRemap.size === 1
        const singleInputMap = isSingleSource
          ? Array.from(sourcesToRemap.values())[0]
          : null
        const canSimplyReplace =
          singleInputMap && singleInputMap.sources.length === 1
        ...
        if (canSimplyReplace && singleInputMap) {
          ...
          const remapped = remapping(
            chunk.map as unknown as RawSourceMap,
            file => {
          ...
          newMappings =
            typeof remapped.mappings === 'string' ? remapped.mappings : ''
