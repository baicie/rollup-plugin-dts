...
const RESOLVED_MODULE_COMMENT = new RegExp(
  `\\/\\*${RESOLVED_MODULE_PREFIX}(.+?)\\*\\/`,
)
...
export class ModuleDeclarationFixer {
  ...
  fix() {
    ...
    for (const node of this.source.statements) {
      ...
      const textBetween = sourceText.slice(
        node.name.getEnd(),
        node.body.getStart(),
      )
      ...
      const quote =
        ...
        'singleQuote' in node.name &&
        node.name.singleQuote
          ? "'"
          : '"'
  ...
  private getTargetChunkName(absolutePath: string): string {
    ...
    const basePath = jsExtMatch
      ? absolutePath.slice(0, -jsExtMatch[0].length)
      : absolutePath
